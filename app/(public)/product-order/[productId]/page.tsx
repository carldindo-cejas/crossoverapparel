...
type Player = { id: number; size: string; number: string; surname: string };
...
export default function ProductOrderPage() {
  ...
  const productType = useMemo(() => {
    ...
    return categoryToProductType(cat?.name);
  }, [product, categories]);
  ...
  /* ── Player helpers ── */
  function addPlayer() {
    setPlayers((prev) => [...prev, { id: Date.now(), size: "M", number: "", surname: "" }]);
  }
  ...
  const sortedPlayers = useMemo(
    () => [...players].sort((a, b) => SIZES.indexOf(a.size as any) - SIZES.indexOf(b.size as any)),
    [players]
  );
  ...
  async function submitOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const orderPayload = {
        ...
        items: [
          {
            ...
            customizations: [
              ...
              { customizationType: "text" as const, fieldName: "playerList", fieldValue: playerListValue, additionalCostCents: 0 },
              { customizationType: "other" as const, fieldName: "otherInstructions", fieldValue: form.otherInstructions || "N/A", additionalCostCents: 0 },
        ...
        notes: `Product order for ${form.teamName} — ${productLabel}`,
      };
      ...
      for (const { file, label } of filesToUpload) {
        ...
        await fetch("/api/uploads", { method: "POST", body: uploadData });
      }
        ...
        {product && (
          <div className="mb-8 overflow-hidden rounded-2xl border border-neutral-200 bg-white md:flex">
            ...
            <div className="flex flex-col justify-center gap-3 p-6">
              <p className="text-xs font-medium uppercase tracking-wider text-neutral-500">Product</p>
              <h1 className="text-2xl font-semibold text-neutral-900">{product.sku} — {product.name}</h1>
              {productType && (
                <span className="inline-block w-fit rounded-full bg-neutral-100 px-3 py-1 text-sm font-medium text-neutral-700">
                  {productType}
                </span>
              )}
              <p className="text-xl font-bold text-neutral-900">{formatCurrency(unitPriceCents || product.base_price_cents, product.currency)}</p>
              {product.description && (
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  ...
                  <span className="inline-block rounded-full bg-yellow-100 px-3 py-1 text-sm font-semibold text-yellow-800">{orderSummary.status}</span>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-neutral-900">{orderSummary.cuttingFront} / {orderSummary.cuttingBack} / {orderSummary.cuttingBelow}</p>
                </div>
                <div className="space-y-1">
            ...
            <CardContent>
              <form className="grid gap-6" onSubmit={submitOrder}>
                ...
                <fieldset className="space-y-4 rounded-xl border border-neutral-200 p-4">
                  <legend className="px-2 text-sm font-semibold text-neutral-700">Cutting Styles</legend>
                  <div className="grid gap-5 md:grid-cols-3">
                    <div className="space-y-2">
                      <Label htmlFor="cuttingFront">Front</Label>
                      <select id="cuttingFront" className="flex h-10 w-full rounded-xl border border-neutral-300 bg-white px-3 text-sm" value={form.cuttingFront} onChange={(e) => updateField("cuttingFront", e.target.value)}>
                    ...
                    <div className="space-y-2">
                      ...
                      <select id="cuttingBack" className="flex h-10 w-full rounded-xl border border-neutral-300 bg-white px-3 text-sm" value={form.cuttingBack} onChange={(e) => updateField("cuttingBack", e.target.value)}>
                    ...
                    <div className="space-y-2">
                      ...
                      <select id="cuttingBelow" className="flex h-10 w-full rounded-xl border border-neutral-300 bg-white px-3 text-sm" value={form.cuttingBelow} onChange={(e) => updateField("cuttingBelow", e.target.value)}>
                ...
                {/* Player List */}
                <fieldset className="space-y-4 rounded-xl border border-neutral-200 p-4">
                  <legend className="px-2 text-sm font-semibold text-neutral-700">Player List</legend>
                  <p className="text-xs text-neutral-500">Players will be sorted by size (smallest → biggest).</p>
                  <div className="space-y-3">
                    {players.map((player) => (
                      <div key={player.id} className="flex flex-wrap items-end gap-3 rounded-lg border border-neutral-100 bg-neutral-50 p-3">
                        <div className="w-20 space-y-1">
                          <Label className="text-xs">Size</Label>
                          <select className="flex h-9 w-full rounded-lg border border-neutral-300 bg-white px-2 text-sm" value={player.size} onChange={(e) => updatePlayer(player.id, "size", e.target.value)}>
                            {SIZES.map((s) => (<option key={s}>{s}</option>))}
                          </select>
                        </div>
                        <div className="w-20 space-y-1">
                          <Label className="text-xs">Number</Label>
                          <Input className="h-9" value={player.number} onChange={(e) => updatePlayer(player.id, "number", e.target.value)} placeholder="#" />
                        </div>
                        <div className="flex-1 space-y-1">
                          <Label className="text-xs">Surname</Label>
                          <Input className="h-9" value={player.surname} onChange={(e) => updatePlayer(player.id, "surname", e.target.value)} placeholder="Last name" />
                        </div>
                        <Button type="button" variant="outline" size="sm" className="h-9 text-red-500" onClick={() => removePlayer(player.id)}>✕</Button>
                      </div>
                    ))}
                  </div>
                  <Button type="button" variant="outline" size="sm" onClick={addPlayer}>+ Add Player</Button>
                </fieldset>

                {/* Logo Attachments */}
                <fieldset className="space-y-4 rounded-xl border border-neutral-200 p-4">
                  <legend className="px-2 text-sm font-semibold text-neutral-700">
                    Logo Attachments <span className="text-xs font-normal text-neutral-400">(max 2MB each)</span>
                  </legend>
                  <div className="grid gap-5 md:grid-cols-3">
                    <div className="space-y-2">
                      <Label htmlFor="leftLogo">Left Logo</Label>
                      <Input id="leftLogo" type="file" accept="image/*" onChange={(e) => setLeftLogo(e.target.files?.[0] || null)} />
                      {leftLogo && fileErr(leftLogo) && <p className="text-xs text-red-600">{fileErr(leftLogo)}</p>}
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="rightLogo">Right Logo</Label>
                      <Input id="rightLogo" type="file" accept="image/*" onChange={(e) => setRightLogo(e.target.files?.[0] || null)} />
                      {rightLogo && fileErr(rightLogo) && <p className="text-xs text-red-600">{fileErr(rightLogo)}</p>}
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="backLogo">Back Logo</Label>
                      <Input id="backLogo" type="file" accept="image/*" onChange={(e) => setBackLogo(e.target.files?.[0] || null)} />
                      {backLogo && fileErr(backLogo) && <p className="text-xs text-red-600">{fileErr(backLogo)}</p>}
                    </div>
                  </div>
                </fieldset>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="otherInstructions" placeholder="Colors, special notes, etc." value={form.otherInstructions} onChange={(e) => updateField("otherInstructions", e.target.value)} />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select id="paymentMethod" className="flex h-10 w-full rounded-xl border border-neutral-300 bg-white px-3 text-sm" value={form.paymentMethod} onChange={(e) => updateField("paymentMethod", e.target.value)}>
                ...
                {/* Map Pin Location */}
                <fieldset className="space-y-4 rounded-xl border border-neutral-200 p-4">
                  <legend className="px-2 text-sm font-semibold text-neutral-700">Map Pin Location *</legend>
                  <p className="text-xs text-neutral-500">Click the map to set your delivery pin, or enter coordinates manually. This is required.</p>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-1">
                      <Label className="text-xs">Latitude</Label>
                      <Input value={form.mapLat} onChange={(e) => updateField("mapLat", e.target.value)} placeholder="e.g. 9.3068" />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs">Longitude</Label>
                      <Input value={form.mapLng} onChange={(e) => updateField("mapLng", e.target.value)} placeholder="e.g. 123.8854" />
                    </div>
                  </div>
                  <MapLoader lat={form.mapLat} lng={form.mapLng} onPick={(lat, lng) => { updateField("mapLat", lat.toFixed(6)); updateField("mapLng", lng.toFixed(6)); }} />
                </fieldset>
